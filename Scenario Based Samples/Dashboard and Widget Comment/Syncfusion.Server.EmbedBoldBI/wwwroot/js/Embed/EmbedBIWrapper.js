...
class BoldBI {
    ...
    */
    ...
    _renderDashboard(responseInfo) {
        ...
        if (!responseInfo.Status) {
            ...
            if (this.embedOptions.pinboardName !== "" && this.pinboardIds.length === 0) {
                ...
                if (this.embedOptions.mode == "view") {
                    ...
                    this.token = embedResponse.access_token;
                }
                var dashboardName = "";
    ...
    _onBoldBIDashboardBeforeBannerIconRender(arg) {
        ...
        if (this.embedOptions.dashboardSettings.enableComment) {
            var commentAndView = {
                groupId: "dashboard-comment-view",
                groupName: "Dashboard Comment & Views",
                items: [
                    this._createBannerIcon("<div/>", "dashboard-comment", "su su-without-comment", "Comment", "dashboardcomment", true, false, { "font-size": "15px" })
                ]
            };
            arg.iconsinformation.unshift(commentAndView);
            //commentAndView.items.push(createBannerIcon("<div/>", "dashboard-comment", "su su-without-comment", "Comment", "dashboardcomment", true, false, { "font-size": "15px" }));
        }
    ...
    _onBoldBIDashboardBannerIconClick(arg) {
        ...
        if (arg.name.toLowerCase() == 'dashboardcomment') {
            this.CommentsArgument = arg;
            if (arg.model.dashboardSettings) {
                this._getComments("dashboard", arg.model.itemId, arg.model.dashboardSettings.parentId, arg.model.dashboardSettings.parentId,1);
            }
            else {
                this._getComments("dashboard", arg.model.itemId, arg.model.itemId,null,1);
            }
        }
    ...
    _getComments(commentType, itemId, dashboardId, parentItemId,orderBy) {
        ...
        this.SortingBy = orderBy;
        if (commentType == "widget") {
           var widgetCmtId = itemId;
        }
        bbEmbed.ajax({
            async: false,
            type: 'POST',
            ...
            success: function (result) {
                if (result.Status) {
                    ...
                    if (commentType == "dashboard") {
                        clientFnc = window[that.embedOptions.dashboardSettings.getDashboardComment];
                        that.CommentsArgument["StatusMessage"] = "Getting comments successfully";
                        that.CommentsArgument["Comments"] = result.Comments;
                        that.CommentsArgument["ActiveCommentsCount"] = result.ActiveCommentsCount;
                        that.CommentsArgument["SortBy"] = that.SortingBy;
                        if (clientFnc instanceof Function) {
                            clientFnc.call(that, that.CommentsArgument);
                        }
                    }
                    else {
                        clientFnc = window[that.embedOptions.widgetSettings.getWidgetComment];
                        that.CommentsArgument;
                        var widgetContainer = bbEmbed("[data-widget-id='" + widgetCmtId+ "']");
                        ...
                        that.CommentsArgument["StatusMessage"] = result.StatusMessage;
                        that.CommentsArgument["Comments"] = result.Comments;
                        that.CommentsArgument["positionY"] = positionY + 20;
                        /*that.widgetArgument["positionY"] = that.widgetArgument.target.offset().top;*/
                        that.CommentsArgument["right"] = right + 40;
                       /* that.widgetArgument["right"] = that.widgetArgument.target.offset().left;*/
                        that.CommentsArgument["widgetId"] = widgetCmtId;
                        that.CommentsArgument["ActiveCommentsCount"] = result.ActiveCommentsCount;
                        that.CommentsArgument["SortBy"] = that.SortingBy;
                        if (clientFnc instanceof Function) {
                            clientFnc.call(that, that.CommentsArgument);
        ...
        /*Object.keys(that.widgetArgument).forEach(k => delete that.widgetArgument[k]);*/
    }

    addComment(parentId, commentedText, commentType, itemId, dashboardId, parentItemId, clientFnc) {
        var that = this;
        ...
        bbEmbed.ajax({
            ...
            success: function (result) {
                if (result.Status) {
                    that._getComments(commentType, itemId, dashboardId, parentItemId,1);
                    //if (window[clientFnc] instanceof Function) {
                    //    window[clientFnc].call(that, that.CommentsArgument);
                    //} else {
                    //    clientFnc.call(that, that.CommentsArgument);
                    //}
                }
    ...
    deleteComment(commentId, commentType, itemId, dashboardId, parentItemId, clientFnc) {
        var that = this;
    ...
    editComment(commentId, editedText, commentType, itemId, dashboardId, parentItemId, clientFnc) {
        var that = this;
    ...
    _onBoldBIDashboardBeforeWidgetIconRendered(arg) {
        ...
        if (this.embedOptions.widgetSettings.enableComment === true && this.embedOptions.pinboardName=="") {
            arg.iconsinformation.push({
                "class": arg.enableComment ? "su-without-comment su-icon" : "",
                "name": "comment",
                "tooltip": ""
            });
        }
    ...
    _onBoldBIDashboardWidgetIconClick(arg) {
        if (arg.name == "comment") {
            this.CommentsArgument = arg;
            if (arg.model.dashboardSettings) {
                this._getComments("widget", arg.widgetId, arg.model.itemId, arg.model.dashboardSettings.parentId,1);
            }
            else {
                this._getComments("widget", arg.widgetId, arg.model.itemId, null,1);
            }
        }

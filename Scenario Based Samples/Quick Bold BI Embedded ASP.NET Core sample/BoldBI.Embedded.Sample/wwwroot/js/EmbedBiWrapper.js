...
class BoldBI {
    ...
    resizeDashboard(filterParameters) {
        var existingDashboardInstance = this._getDashboardInstance();
        if (existingDashboardInstance != undefined) {
            var clientFnc = window[this.embedOptions.onResize];
        ...
        if (!responseInfo.Status) {
            ...
            if (this.embedOptions.pinboardName !== "" && this.pinboardIds.length === 0) {
                ...
                if (this.pinboardIds.length > 0) {
                    ...
                    embedContainerId = 'multi_' + embedResponse.ItemDetail.Id;
                } else {
                    embedContainerId = this.embedOptions.embedContainerId;
                }
                var height = this.pinboardIds.length > 0 ? $('#' + embedContainerId).height() : (!ej.isNullOrUndefined(document.getElementById(this.embedOptions.embedContainerId)) ? this.embedOptions.height : (parseInt(this.embedOptions.height) - 36 + 'px'));
                ...
                if (window.jQuery instanceof Function) {
                    var embedContainer = window.jQuery.call(that, "#" + (this.pinboardIds.length > 0 ? (embedContainerId + '_embeddedbi') : that.childContainer.id));
                    var embedChildId;
                    ...
                    if (column[i].Item.length > 0) {
                        $.each(column[i].Item, function (j) {
                            ...
                            if (column[i].Item[j].ItemExtension.toLowerCase() !== ".sydj") {
                                ...
                                var deleteIconDiv = that.embedOptions.enableUnpinWidget ? '<div id="widget-icons"><i class="items unpin-widget su su-delete" data-toggle="tooltip" data-original-title="Unpin Widget"  style="color: black;" /></div>' : '';
                                $('#column-' + (i + 1)).append('<li class="list-item" ' + styleAttr + '><div class="widget" id=widget_' + (i + 1) + '_' + (j + 1) + ' data-dashboardurl="' + href + '" style="height: ' + height + 'px;width:100%;background:#fff;"><div class="widget-sortable" style="width:100%;float:left;display:block;height:25px"><div style="height:100%;width:100%;cursor:move;"><div id="item-name">' + itemName + '</div>' + deleteIconDiv +'</div></div></div></li>');
                            }
    ...
    _addWidgetInPinboard(itemDetails) {
        ...
        var deleteIconDiv = this.embedOptions.enableUnpinWidget ? '<div id="widget-icons"><i class="items unpin-widget su su-delete" data-toggle="tooltip" data-original-title="Unpin Widget"  style="color: black;" /></div>' : '';
        $(ulElement).prepend('<li class="list-item"><div class="widget" id=widget_' + 1 + '_' + (ulElementLilength + 1) + ' data-dashboardurl="' + href + '" style="height: ' + height + 'px;width:100%;background:#fff;"><div class="widget-sortable" style="width:100%;float:left;display:block;height:25px"><div style="height:100%;width:100%;cursor:move;"><div id="item-name">' + itemName + '</div>' + deleteIconDiv +'</div></div></div></li>');
    ...
    _renderMultiTabDashboard(embedResponse) {
        ...
        $(".e-tab-header")[0].ej2_instances[0].refreshOverflow();

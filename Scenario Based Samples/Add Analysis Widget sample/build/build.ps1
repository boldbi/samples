...
try
{
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`"  -target=`"$Target`" -outputpath=`"$OutputPath`" -referencepath=`"$ReferencePath`" -PreReleaseNumber=`"$revisionNumber`" -assemblyfileversion=`"$AssemblyFileVersion`" -configuration=`"$Configuration`" -studio_version=`"$StudioVersion`" -nugetserverurl=`"$nugetserverUrl`" -nugetapikey=`"$nugetapiKey`" -verbosity=`"$Verbosity`" $ScriptArgs"
}
Catch
{   
throw "Build Failed"
